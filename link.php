<link rel="stylesheet" href="libreria/bootstrap/css/bootstrap.min.css">
<link rel="stylesheet" href="vistas/css/estils.css">

<script src="libreria/jquery/jquery-3.3.1.min.js"></script>
<script src="libreria/bootstrap/js/bootstrap.min.js"></script>
<script src="libreria/froalaEditor/js/froala_editor.min"></script>
<script src="libreria/froalaEditor/js/froala_editor.pkgd.min"></script>
<script src="controller/js/login.js"></script>
<script src="controller/js/navbar.js"></script>
